...
| モード | ショートカット | 動作 |
|---|---|---|
| **分割ビュー** | `F3` | 左に Markdown ソース、右にライブ同期プレビュー |
| **インライン編集** | `F2` | 読書キャンバス上でその場編集 |
| **読書モード** | `Esc` | クリーンな読書に戻る、またはクリーンなら終了 |

- `Cmd/Ctrl+S` で保存。**改行（LF/CRLF）とエンコーディング（UTF-8/BOM）を忠実に保持**。
- 未保存の変更は保護されます——閉じる・終了・外部変更で静かにデータを失うことはありません。
- 分割ビューはカーソル行をプレビュー側で揃えてハイライト。
- すべてのビューが 1 つのドキュメント状態とアンドゥ履歴を共有。

### ワークスペース

- ターミナルで `qvreader .` を実行すると、**フォルダをワークスペース**として開き、ファイルツリーのサイドバーを表示。
- CLI を一度登録すれば、どこからでもファイルやプロジェクトを開けます。
