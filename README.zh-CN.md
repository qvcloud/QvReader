...
QvReader 秉承透明、本地优先且对开源社区友好的商业化模式：

- **100% 客户端源码开源**：
  - 桌面客户端的全部源码（React 界面、CodeMirror 6 编辑器、Tauri 原生 Rust 核心及离线验签逻辑）均遵循 [Apache License 2.0](LICENSE) 许可协议开放。
  - 支持直接克隆后使用 `npm ci && npm run build` 构建，不依赖私有服务或内部包。
  - 本地开发与测试环境（`NODE_ENV !== 'production'`）默认解锁全量 Pro 特性，方便贡献者体验与测试。
- **再分发与商标使用规则**：
  - 允许依据 Apache-2.0 自由 Fork、修改及重新分发。
  - 依据 [TRADEMARKS.md](TRADEMARKS.md)，任何第三方或自行构建版本**必须**在窗口标题与关于对话框中保留 **“Community Build”** 或 **“非官方构建”** 标识，不得混淆官方品牌。
  - 官方数字代码签名（Apple Developer ID 公证、Windows 商业签名）仅属于维护者发布的官方安装包。
- **隐私与本地优先承诺**：
  - 软件完全离线可用，用户的 Markdown 文档内容与本地路径**绝不上传**至任何远程服务器。
  - Pro 授权激活仅传输用户输入的密钥、匿名硬件哈希与客户端版本；授权失效或离线不会影响任何本地文件的打开与保存。
- **社区版与专业版划分（官方分发渠道）**：
  - **社区版**：核心阅读与 F2 编辑永久免费无广告，F3 模式提供充足试用且保存功能永不锁死。
  - **专业版（$9.99 永久买断制）**：一次性付费，支持 3+ 设备，100% 本地离线激活，消除 Pro 提示。
- **技术支持边界**：
  - 官方团队对官方发行版提供完整支持（详见 [SUPPORT.md](SUPPORT.md)），修改后的第三方 Fork 由其分支维护者自行支持。
